document.addEventListener('DOMContentLoaded', function() {
      ...
      document.getElementById('searchDonorBtn').addEventListener('click', async function() {
        ...
        if (!query) {
          alert('Please enter a name, ID, or blood type to search!');
          return;
        }

        try {
          const res = await fetch(`http://127.0.0.1:3000/search-donors?q=${encodeURIComponent(query)}`);
          if (!res.ok) throw new Error(`HTTP error! Status: ${res.status}`);
          ...
          if (donors.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; color: red;">No donors found matching that search.</td></tr>';
          } else {
            donors.forEach(donor => {
              const row = document.createElement('tr');
              row.innerHTML = `
                ...
                <td>${donor.Gender || 'N/A'}</td>
                <td>${donor.Date_of_Birth ? new Date(donor.Date_of_Birth).toLocaleDateString() : 'N/A'}</td>
                <td><strong style="color: #e74c3c;">${donor.Blood_Type || 'N/A'}</strong></td>
                <td>${donor.Phone_Numbers.length > 0 ? donor.Phone_Numbers.join(', ') : 'None'}</td>
              `;
              tbody.appendChild(row);
            });
          ...
          document.getElementById('donorDataSection').scrollIntoView({ behavior: 'smooth' });
        } catch (error) {
